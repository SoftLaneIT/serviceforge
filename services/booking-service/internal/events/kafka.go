...
// brokers is a comma-free slice of "host:port" strings,
// e.g. []string{"kafka:9092"}.
func NewKafka(brokers []string) *KafkaPublisher {
	w := &kafka.Writer{
		...
		// Allow the writer to auto-create topics if the broker permits it.
...
// NoopPublisher discards all events.  Used in tests and when Kafka is
